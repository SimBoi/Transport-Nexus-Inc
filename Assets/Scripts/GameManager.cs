...
public class GameManager : MonoBehaviour
{
    ...
    public void FocusStructure(GameObject structure, bool structureUI = true, bool portUI = true, List<Port> excludePorts = null, bool buildingUI = true)
    {
        ...
        {
            focusTransform = FocusedStructure.GetComponentInChildren<StructureUI>().focusPoint;
        }
        else
        {
            ...
            focusTransform = structure.transform;
            focusOffset = size / 2f * (relativeRight + relativeUp);
        }

        if (enableStructureUI)
        {
            FocusedStructure.GetComponent<StructureUI>().Focus();
        }
        if (enablePortUI)
        {
            HighlightDisconnectedPorts(focusTransform.position + focusOffset, excludePorts: excludePorts);
            HighlightWires(Vector3ToTile(focusTransform.position + focusOffset));
        }
        if (enableBuildingUI)
        {
    ...
    public void Unfocus(bool disableStructureUI = true, bool disablePortUI = true, List<Port> excludePorts = null, bool disableBuildingUI = true, bool disableTrainUI = true, List<ConnectableExtender> excludeExtenders = null)
    {
        ...
        {
            FocusedTrain.GetComponent<TrainUI>().Unfocus();
            FocusedTrain = null;
        }
    }
